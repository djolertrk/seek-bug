...
void ProgressEventThreadFunction() {
  lldb::SBListener listener("seek-bug-dap.progress.listener");
...
void request_attach(const llvm::json::Object &request) {
  ...
  // the seek-bug-dap binary to have its working directory set to that relative
...
void request_initialize(const llvm::json::Object &request) {
  ...
  auto cmd = g_dap.debugger.GetCommandInterpreter().AddMultiwordCommand(
      "seek-bug-dap", "Commands for managing seek-bug-dap.");
  ...
  cmd.AddCommand(
      ...
      "Get or set the repl behavior of seek-bug-dap evaluation requests.");
...
void request_launch(const llvm::json::Object &request) {
  ...
  // require the seek-bug-dap binary to have its working directory set to that
...
void request_handleAIRequest(const llvm::json::Object &request) {
  // Look for the "arguments" key.
  auto argsIt = request.find("arguments");
  if (argsIt == request.end() || !argsIt->second.getAsObject()) {
    llvm::json::Object errorResponse{
        {"type", "response"},
        {"command", "aiRequest"},
        {"success", false},
        {"message", "Missing or invalid arguments."}
    };
    g_dap.SendJSON(llvm::json::Value(std::move(errorResponse)));
    return;
  }
  // Get the arguments object.
  const llvm::json::Object *argsPtr = argsIt->second.getAsObject();
  const llvm::json::Object &args = *argsPtr;
  
  // Retrieve the AI command string.
  std::string aiCmd;
  auto cmdIt = args.find("command");
  if (cmdIt != args.end()) {
    if (auto sOpt = cmdIt->second.getAsString())
      aiCmd = std::string(*sOpt);
  }
  ...
  if (aiCmd.empty()) {
    llvm::json::Object errorResponse{
        ...
        {"command", "aiRequest"},
        {"success", false},
        {"message", "No AI command specified."}
    };
    g_dap.SendJSON(llvm::json::Value(std::move(errorResponse)));
    return;
  }
  
  // Prepend "ai " so that LLDB routes the command to your AICommands plugin.
  std::string fullCmd = "ai " + aiCmd;
  
  lldb::SBCommandInterpreter interpreter = g_dap.debugger.GetCommandInterpreter();
  lldb::SBCommandReturnObject cmdResult;
  interpreter.HandleCommand(fullCmd.c_str(), cmdResult);
  
  llvm::json::Object response{
      {"type", "response"},
      {"command", "aiRequest"},
      {"success", cmdResult.Succeeded()},
      {"body", llvm::json::Object{{"output", std::string(cmdResult.GetOutput())}}}
  };
  g_dap.SendJSON(llvm::json::Value(std::move(response)));
}
...
void RegisterRequestCallbacks() {
  ...
  g_dap.RegisterRequestCallback("aiRequest", request_handleAIRequest);
}
...
static void printHelp(LLDBDAPOptTable &table, llvm::StringRef tool_name) {
  ...
  table.printHelp(llvm::outs(), usage_str.c_str(), "SEEK-BUG DAP", false);

  std::string examples = R"___(
EXAMPLES:
  The debug adapter can be started in two modes.

  Running seek-bug-dap without any arguments will start communicating with the
  parent over stdio. Passing a port number causes seek-bug-dap to start listening
...
// If --launch-target is provided, this instance of seek-bug-dap becomes a
